...
const FooterToolsInitiator = {
  ...
  async _subscribePushMessage(event) {
    ...
    if (!(await this._isNotificationReady())) {
      console.log('Notifacation isn\'t ready');
      return;
    }
    ...
    try {
      ...
      await pushSubscription?.unsubscibe();
    }
  },
  ...
  async sendPostToServer(url, data) {
  ...
  async _isNotificationReady() {
    ...
    if (!NotificationHelper._checkPermission()) {
      console.log('User did not granted the notification persmission yet!');
