...
const RENDER_UI_EDITED_ARTICLE_BOOK = 'render-ui-edited-book';
...
function saveEditedBook(book, newTitle, newAuthor, newYear, newStatus) {
  ...
  saveData();
  document.dispatchEvent(new Event(RENDER_UI_EDITED_ARTICLE_BOOK));
  document.dispatchEvent(new Event(RENDER_EVENT_UI));
  // document.dispatchEvent(new Event(RENDER_SEARCH_UI));
}
...
document.addEventListener(RENDER_UI_EDITED_ARTICLE_BOOK, function () {});
